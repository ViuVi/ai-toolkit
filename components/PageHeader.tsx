...
export default function PageHeader() {
  ...
  const handleHomeClick = () => {
    // Ana sayfadaki scroll pozisyonunu kaydet
    if (typeof window !== 'undefined') {
      const savedPosition = sessionStorage.getItem('homeScrollPosition')
      if (savedPosition) {
        // Biraz gecikme ile scroll et (sayfa yüklendikten sonra)
        setTimeout(() => {
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[#0a0a0f]/90 backdrop-blur-xl border-b border-white/5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <Link href="/" onClick={handleHomeClick} className="text-gray-400 hover:text-white transition text-sm">{t.home}</Link>
