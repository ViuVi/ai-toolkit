...
function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      const initialCredits = referralCode ? 150 : 100
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white flex">
      <div className="flex-1 flex flex-col justify-center px-8 md:px-16 lg:px-24">
        <div className="max-w-md w-full mx-auto">
          ...
          {referralCode && (
            <div className="p-3 bg-green-500/10 border border-green-500/20 rounded-xl text-green-400 text-sm mb-6">
              🎁 Referral: <span className="font-mono font-bold">{referralCode}</span> (+50 bonus!)
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {!referralCode && (
              <div>
                ...
                <input type="text" value={referralCode} onChange={e => setReferralCode(e.target.value.toUpperCase())} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white font-mono focus:outline-none focus:border-purple-500/50" placeholder="XXXXXXXX" maxLength={8} />
              </div>
            )}
      ...
      <div className="hidden lg:flex flex-1 bg-gradient-to-br from-purple-500/10 via-pink-500/10 to-purple-500/10 items-center justify-center p-12 relative overflow-hidden">
        ...
        <div className="relative text-center max-w-md">
          ...
          <p className="text-gray-400 text-lg">Start creating viral content today</p>
        </div>
      </div>
    </div>
  )
