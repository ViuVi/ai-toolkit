      ...
      <div className="max-w-md w-full">
        ...
        <div className="bg-gray-800 rounded-2xl p-8 border border-gray-700">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-400">{auth.loginSubtitle}</p>
          </div>
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="text-right"><a href="#" className="text-sm text-purple-400 hover:text-purple-300">{auth.forgot}</a></div>
            <button type="submit" disabled={loading} className="w-full py-3 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 rounded-xl font-semibold text-white transition disabled:opacity-50">{loading ? '...' : auth.loginBtn}</button>
          </form>
          <p className="text-center text-gray-400 mt-6">{auth.noAccount} <Link href="/register" className="text-purple-400 hover:text-purple-300">{auth.createOne}</Link></p>
        </div>
      </div>
    </div>
  )
