...
const toolTexts: Record<string, Record<string, string>> = {
  ...
  fr: { title: 'Chercheur de vidéos virales', niche: 'Niche', nichePlaceholder: 'ex: Fitness, Tech...', platform: 'Plateforme', generate: 'Trouver des idées virales', generating: 'Recherche...', emptyTitle: 'Idées vidéo', emptyDesc: 'Entrez votre niche' }
}
...
export default function VideoFinderPage() {
  ...
  const handleSearch = async () => {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/video-finder', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ niche, platform, language, userId: user.id })
      })
          ...
          <div className="lg:col-span-2">
            <div className="p-6 bg-white/[0.02] border border-white/5 rounded-2xl space-y-5">
              <div>
                ...
                <input type="text" value={niche} onChange={(e) => setNiche(e.target.value)} placeholder={t.nichePlaceholder || "e.g: Fitness, Tech..."} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-purple-500/50" />
              </div>
              <div>
              ...
              <button onClick={handleSearch} disabled={loading || !niche.trim()} className="w-full py-4 bg-gradient-to-r from-purple-500 to-pink-500 rounded-xl font-semibold disabled:opacity-50 flex items-center justify-center gap-2">
                {loading ? <><span className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></span>{t.generating}</> : '🔍 Viral Fikirler Bul'}
              </button>
