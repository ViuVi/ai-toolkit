      ...
      <header className="sticky top-0 z-50 bg-[#0a0a0f]/80 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-bold">🎠 Carousel Planner</h1>
          </div>
        ...
        <div className="grid lg:grid-cols-5 gap-8">
          <div className="lg:col-span-2">
            <div className="p-6 bg-white/[0.02] border border-white/5 rounded-2xl space-y-5">
              <div>
                <label className="block text-sm text-gray-400 mb-2">Carousel Konusu</label>
                <input type="text" value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="örn: 5 adımda viral olmak..." className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-purple-500/50" />
              </div>
              <div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[['educational', '📚 Eğitici'], ['storytelling', '📖 Hikaye'], ['listicle', '📋 Liste'], ['how-to', '🎯 Nasıl Yapılır']].map(([val, label]) => (
              ...
              <button onClick={handleGenerate} disabled={loading || !topic.trim()} className="w-full py-4 bg-gradient-to-r from-purple-500 to-pink-500 rounded-xl font-semibold disabled:opacity-50 flex items-center justify-center gap-2">
                {loading ? <><span className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></span>Planlanıyor...</> : '🎠 Carousel Oluştur'}
              </button>
              {error && <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-sm">{error}</div>}
            </div>
          </div>
          <div className="lg:col-span-3 space-y-4 max-h-[calc(100vh-150px)] overflow-y-auto">
            {!result && !loading && <div className="p-12 bg-white/[0.02] border border-white/5 rounded-2xl text-center"><div className="text-5xl mb-4">🎠</div><h3 className="text-xl font-medium mb-2">Carousel Planner</h3><p className="text-gray-500">Konunuzu girin, slide'ları planlayalım</p></div>}
              ...
              <div className="space-y-4">
                ...
                {result.carousel_concept && (
                  <div className="p-4 bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-500/20 rounded-xl">
                    <h4 className="font-semibold text-purple-400 mb-2">💡 Konsept</h4>
                ...
                {result.caption && (
                  <div className="p-4 bg-white/[0.02] border border-white/5 rounded-xl">
                    <div className="flex justify-between items-center mb-3">
                      <h4 className="font-semibold text-purple-400">✍️ Caption</h4>
                      <button onClick={copyCaption} className="px-3 py-1 bg-purple-500/20 text-purple-400 rounded text-sm">{copied ? '✓' : 'Kopyala'}</button>
                    </div>
                ...
                {result.design_specs && (
                  <div className="p-4 bg-white/[0.02] border border-white/5 rounded-xl">
                    <h4 className="font-semibold text-purple-400 mb-2">📐 Tasarım</h4>
