...
const toolTexts: Record<string, Record<string, string>> = {
  tr: { title: 'Akıllı Paylaşım Saatleri', niche: 'Niş', nichePlaceholder: 'örn: Moda, Fitness...', targetAudience: 'Hedef Kitle (opsiyonel)', audiencePlaceholder: 'örn: 18-25 yaş kadınlar...', platforms: 'Platformlar', generate: 'En İyi Zamanları Bul', generating: '{t.generating}', emptyTitle: 'Posting Optimizer', emptyDesc: '{t.emptyDesc}', bestTimes: 'En İyi Zamanlar', weeklySchedule: 'Haftalık Program', proTips: 'Pro İpuçları' },
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const res = await fetch('/api/posting-optimizer', {
        ...
        body: JSON.stringify({ niche, targetAudience, platforms: platforms.join(','), language, userId: user.id })
      })
          ...
          <div className="lg:col-span-2">
            <div className="p-6 bg-white/[0.02] border border-white/5 rounded-2xl space-y-5">
              ...
              <button onClick={handleAnalyze} disabled={loading || !niche.trim()} className="w-full py-4 bg-gradient-to-r from-purple-500 to-pink-500 rounded-xl font-semibold disabled:opacity-50 flex items-center justify-center gap-2">
                {loading ? <><span className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></span>{t.generating}</> : '⏰ En İyi Zamanları Bul'}
              </button>
