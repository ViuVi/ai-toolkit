...
const texts: Record<Language, any> = {
  en: { back: '← Back', title: 'Trend Detector', subtitle: 'Discover trending topics in your niche', credits: '5 Credits', nicheLabel: 'Niche', nichePlaceholder: 'e.g. fitness, tech...', platformLabel: 'Platform', platforms: { instagram: 'Instagram', tiktok: 'TikTok', youtube: 'YouTube', twitter: 'Twitter/X' }, regionLabel: 'Region', regions: { global: 'Global', us: 'USA', eu: 'Europe', asia: 'Asia' }, detect: 'Detect Trends', detecting: 'Detecting...', results: 'Trending Topics', trendScore: 'Score', growth: 'Growth', emptyInput: 'Enter niche', success: 'Done!', error: 'Error' },
  tr: { back: '← Geri', title: 'Trend Dedektörü', subtitle: 'Nişinizdeki trend konuları keşfedin', credits: '5 Kredi', nicheLabel: 'Niş', nichePlaceholder: 'örn: fitness, teknoloji...', platformLabel: 'Platform', platforms: { instagram: 'Instagram', tiktok: 'TikTok', youtube: 'YouTube', twitter: 'Twitter/X' }, regionLabel: 'Bölge', regions: { global: 'Global', us: 'ABD', eu: 'Avrupa', asia: 'Asya' }, detect: 'Trendleri Bul', detecting: 'Aranıyor...', results: 'Trend Konular', trendScore: 'Skor', growth: 'Büyüme', emptyInput: 'Niş girin', success: 'Tamamlandı!', error: 'Hata' },
  ru: { back: '← Назад', title: 'Детектор трендов', subtitle: 'Находите трендовые темы', credits: '5 кредитов', nicheLabel: 'Ниша', nichePlaceholder: 'напр: фитнес...', platformLabel: 'Платформа', platforms: { instagram: 'Instagram', tiktok: 'TikTok', youtube: 'YouTube', twitter: 'Twitter/X' }, regionLabel: 'Регион', regions: { global: 'Глобально', us: 'США', eu: 'Европа', asia: 'Азия' }, detect: 'Найти', detecting: 'Поиск...', results: 'Тренды', trendScore: 'Рейтинг', growth: 'Рост', emptyInput: 'Введите нишу', success: 'Готово!', error: 'Ошибка' },
  de: { back: '← Zurück', title: 'Trend-Detektor', subtitle: 'Entdecken Sie Trendthemen', credits: '5 Credits', nicheLabel: 'Nische', nichePlaceholder: 'z.B. Fitness...', platformLabel: 'Plattform', platforms: { instagram: 'Instagram', tiktok: 'TikTok', youtube: 'YouTube', twitter: 'Twitter/X' }, regionLabel: 'Region', regions: { global: 'Global', us: 'USA', eu: 'Europa', asia: 'Asien' }, detect: 'Finden', detecting: 'Suche...', results: 'Trends', trendScore: 'Score', growth: 'Wachstum', emptyInput: 'Nische eingeben', success: 'Fertig!', error: 'Fehler' },
  fr: { back: '← Retour', title: 'Détecteur de tendances', subtitle: 'Découvrez les sujets tendance', credits: '5 crédits', nicheLabel: 'Niche', nichePlaceholder: 'ex: fitness...', platformLabel: 'Plateforme', platforms: { instagram: 'Instagram', tiktok: 'TikTok', youtube: 'YouTube', twitter: 'Twitter/X' }, regionLabel: 'Région', regions: { global: 'Global', us: 'USA', eu: 'Europe', asia: 'Asie' }, detect: 'Détecter', detecting: 'Recherche...', results: 'Tendances', trendScore: 'Score', growth: 'Croissance', emptyInput: 'Entrez niche', success: 'Terminé!', error: 'Erreur' }
}
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="bg-gray-800/50 rounded-2xl border border-gray-700 p-6 mb-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-medium text-gray-300 mb-2">{t.platformLabel}</label><select value={platform} onChange={(e) => setPlatform(e.target.value)} className="w-full px-4 py-3 bg-gray-900/50 border border-gray-700 rounded-xl text-white">{Object.entries(t.platforms).map(([k, v]) => (<option key={k} value={k}>{v as string}</option>))}</select></div><div><label className="block text-sm font-medium text-gray-300 mb-2">{t.regionLabel}</label><select value={region} onChange={(e) => setRegion(e.target.value)} className="w-full px-4 py-3 bg-gray-900/50 border border-gray-700 rounded-xl text-white">{Object.entries(t.regions).map(([k, v]) => (<option key={k} value={k}>{v as string}</option>))}</select></div></div>
        </div>
        ...
        {result && (
          <div className="bg-gray-800/50 rounded-2xl border border-gray-700 p-6"><h2 className="text-xl font-semibold mb-6 flex items-center gap-2"><span>🔥</span>{t.results}</h2><div className="space-y-4">{result.trends?.map((trend: any, i: number) => (
            <div key={i} className="bg-gray-900/50 rounded-xl p-5 border border-gray-700 hover:border-purple-500/50 transition">
              <div className="flex items-start justify-between mb-3"><div className="flex items-center gap-3"><span className="w-8 h-8 bg-gradient-to-r from-purple-600 to-pink-600 rounded-full flex items-center justify-center text-sm font-bold">{i + 1}</span><h3 className="font-semibold text-lg">{trend.topic}</h3></div><span className="text-green-400 font-semibold">{trend.growth}</span></div>
              <div className="flex items-center gap-4"><span className="text-sm text-gray-400">{t.trendScore}:</span><div className="w-24 h-2 bg-gray-700 rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-purple-500 to-pink-500" style={{ width: `${trend.trendScore}%` }}></div></div><span className="text-sm font-medium text-purple-400">{trend.trendScore}%</span></div>
            </div>
          ))}</div></div>
        )}
      </main>
