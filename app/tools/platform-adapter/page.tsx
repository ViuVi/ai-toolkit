...
export default function ToolPage() {
  ...
  const title = toolNames[language].platformAdapter
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch('/api/platform-adapter', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ input, userId, language }) })
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <span className="inline-block px-3 py-1 bg-purple-500/20 text-purple-400 text-sm rounded-full mb-4">⚡ 3 Credits</span>
